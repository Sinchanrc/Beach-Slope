module solver
    ...
    ! subroutine cg(tls,guess,ls,val,row,col,bvecs,sl) ! Conjugate gradient solver
    !     implicit none
    !     real(dp),intent(in) :: bvecs(:),val(:)
    !     integer,intent(in) :: ls
    !     real(dp) :: rs(ls),p(ls),res(ls),alpha,betas,mm1(ls),prevr,error
    !     integer,intent(in),dimension(:) :: row(ls+1),col(:)
    ...
    !     !$omp parallel do default(shared) private(ks) schedule(static)
    !         do ks=1,ls
    !         rs(ks)=bvecs(ks)-res(ks)
    !         p(ks)=rs(ks)
    !         end do
    !     !$omp end parallel do
    ...
    !         call spmv(ls,val,row,col,p,mm1)
    !         prevr=vecvec(ls,rs,rs)
    !         alpha=(prevr/vecvec(ls,mm1,p))

    !         !$omp parallel do default(shared) private(ks) schedule(static)
    !         do ks=1,ls
    !             sl(ks)=guess(ks)+alpha*p(ks)
    !             rs(ks)=rs(ks)-alpha*mm1(ks)
    !         end do
    !         !$omp end parallel do

    !         betas=(vecvec(ls,rs,rs)/prevr)

    !         !$omp parallel do default(shared) private(ks) schedule(static)
    !         do ks=1,ls
    !             p(ks)=rs(ks)+betas*p(ks)
    !         end do
    !         !$omp end parallel do

    !         ! !$omp parallel do default(shared) reduction(+:error) schedule(static)
    !         ! do ks=1,ls 
    !         !     error=error+(((sl(ks)-guess(ks))/guess(ks))**2)/ls
    !         ! end do
    !         ! !$omp end parallel do 

    !         ! if(sqrt(error)>tls) then
    !         !     tcal=1
    !         !     guess=sl
    !         !     error=0.0d0
    !         !     exit
    ...
    !             !     cycle
    !             ! end if
    !             if(abs((sl(ks)-guess(ks))/(guess(ks)+1e-6))>tls) then
    !                 tcal=1
    !                 guess=sl
    !                 exit
    !             else
    ...
    ! subroutine gs(tls,guess,ls,array,bvecs,sl)

    !     implicit none
    !     real(dp),intent(in) :: bvecs(:)
    !     integer,intent(in) :: ls
    !     real(dp) :: rs,p(ls),res(ls),alpha,omega,error
    !     type(matrixsetup) :: array(ls)
    !     real(dp),intent(out) ::  sl(:)
    !     integer :: tcal=1,ks,n,diag
    !     real(dp),intent(in) :: tls
    !     real(dp),intent(inout) :: guess(ls)
    ...
    !             rs=rs+array(n)%val(ks)*sl(array(n)%col(ks))

    !         end do
    !         sl(n)=guess(n)+(bvecs(n)-rs)/(array(n)%val(array(n)%sz))*omega
    !         rs=0.0d0
    !         end do

    !         do ks=1,ls
    !         if(abs((sl(ks)-guess(ks))/(abs(guess(ks))+1e-6))>tls) then
    !             tcal=1
    !             guess=sl
    !             exit
    !         else
