module scalar
    ...
    subroutine scalart
        ...
        integer :: i,j,k,m
        real(dp) :: t1,t2,t3,con1,con2

        !$omp do schedule (runtime) private(m,k,i,j,t1,t2,t3,con1,con2) collapse(2)
            do j=sx,ex
                do i=sy,ey            
                ...
                con1=dpcell(i,j)%plist(k)%mass/dpcell(i,j)%plist(k)%ovol
            ...
            do j=sx,ex
                do i=sy,ey            
                    ...
                    if(dpcell(i,j)%plist(k)%tid==3) then
                        ...
                        dpcell(i,j)%plist(k)%con=dpcell(i,j)%plist(k)%con*stime
            ...
            do j=sx,ex
                do i=sy,ey            
                    ...
                    if(dpcell(i,j)%plist(k)%tid==3) then
                        ...
                        dpcell(i,j)%plist(k)%mass=dpcell(i,j)%plist(k)%mass+dpcell(i,j)%plist(k)%con
